...
$output_board_affil = $fb->form("output_board_affil");
$output_board_affil = $util->getUseAffil($output_board_affil);
$output_board_size  = $fb->form("output_board_size");
...
$param["output_board_affil"] = $output_board_affil;
...
// 각 정보항목 폼
// 맵핑코드|판매채널|출력명|판구분|사이즈|기준단위
$title_form      = "%s|%s|%s|%s|%s|판";
// 기본가격|요율|적용금액|신규가격
$price_form      = "%s|%s|%s|%s";
...
$file_path = $excelLib->createExcelFile("output_sell_price_list");

if (is_file($file_path)) {
    echo "output_sell_price_list";
} else {
    echo "FALSE";
}
