...
// 각 정보항목 폼
// 카테고리명|종이정보|사이즈|페이지 정보|전면인쇄도수|후면인쇄도수|기준수량
$title_form = "%s|%s|%s|%s %sp %s|%s|%s|%s";
...
for ($i = 0; $i < $paper_mpcode_arr_count; $i++) {
    ...
    while ($price_info_rs && !$price_info_rs->EOF) {
        ...
        $page        = $price_info_rs->fields["page"];
        $page_dvs    = $price_info_rs->fields["page_dvs"];
        $page_detail = $price_info_rs->fields["page_detail"];

        $bef_print_name = $price_info_rs->fields["bef_print_name"];
        $aft_print_name = $price_info_rs->fields["aft_print_name"];
