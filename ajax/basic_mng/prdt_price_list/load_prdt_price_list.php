...
function makePaperTotalInfoArr($rs) {
    ...
    while ($rs && !$rs->EOF) {
        ...
        $ret["mpcode"][$i] = $mpcode;
        $ret["affil"][$i]  = $affil;
        $ret["info"][$i++] = sprintf($info_form, $affil
                                               , $name
                                               , $dvs
                                               , $color
                                               , $basisweight);
...
function makePlyInfoArr(&$rs,
                        ...
                        $param) {
    // 각 정보항목 폼
    // 카테고리명|종이정보|사이즈|페이지 정보|전면인쇄도수|전면추가인쇄도수|후면인쇄도수|후면추가인쇄도수|기준수량
    $title_form = "%s|%s|%s|%s %sp %s|%s|%s|%s|%s|%s";
    // $title에 해당하는 식별값들
    // 카테고리 분류코드|종이 맵핑코드|규격 맵핑코드|페이지 정보|전면도수맵핑코드|-|후면도수맵핑코드|-|기준단위
    $title_info_form = "%s|%s|%s|%s!%s!%s|%s|-|%s|-|%s";
