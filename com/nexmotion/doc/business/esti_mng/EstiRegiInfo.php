...
function makeEstiRegiContHtml($param) {
    $html = <<<html
        <div id="esti_cont_ctrl">
				...
				<fieldset class="mt25"  style='display:none'>
                <legend> 종이정보 </legend>
				  $param[paper]
                <div id="paper_ctrl">
                ...
                <legend> 답변내용 </legend>
            		<ul class="form-group">
						...
						<li class="fix_width280 fl">
							...
							<input type="text" class="input_dis_co2 fix_width100" id="supply_price" value="$param[supply_price]" disabled="disabled">
							...
							<input type="text" class="input_dis_co2 fix_width100" id="vat" value="$param[vat]" disabled="disabled">
							<br />
							...
							<input type="text" class="input_dis_co2 fix_width100" id="esti_price" value="$param[esti_price]" disabled="disabled">
							<br />
						</li>
					</ul>
					<br /><br /><br /><br />
...
function makePaperLowHtml($param) {
    ...
    $html = <<<html
					<div class="form-group" id="paper_group$param[idx]">
						<label class="control-label fix_width120 tar">종이</label><label class="fix_width20 fs14 tac">:</label>
						<select class="fix_width130" id="paper_name$param[idx]" onchange="selectOptionVal.exec('paper', 'DVS', '$param[idx]')">
                          $param[paper_option_html]
						</select>
						<select class="fix_width130" id="paper_dvs$param[idx]" onchange="selectOptionVal.exec('paper', 'COLOR', '$param[idx]')">
							<option value="">구분(전체)</option>
						</select>
						<select class="fix_width130" id="paper_color$param[idx]" onchange="selectOptionVal.exec('paper', 'BASISWEIGHT', '$param[idx]')">
							<option value="">색상(전체)</option>
						</select>
						<select class="fix_width130" id="paper_basisweight$param[idx]">
							<option value="">평량(전체)</option>
						</select>
						<label class="fix_width10"></label>
						*
						...
						<label class="fix_width10"></label>
						<input type="text" class="input_co2 fix_width100" id="paper_price$param[idx]">
...
function makeOutputLowHtml($param) {
    ...
    $html = <<<html
					<div class="form-group" id="output_group$param[idx]">
						<label class="control-label fix_width120 tar">출력</label><label class="fix_width20 fs14 tac">:</label>
						<select class="fix_width220" id="output_name$param[idx]" onchange="selectOptionVal.exec('output', 'BOARD', '$param[idx]')">
                          $param[output_option_html]
						</select>
						<select class="fix_width180" id="output_board$param[idx]" onchange="selectOptionVal.exec('output', 'SIZE', '$param[idx]')">
							<option value="">출력판(전체)</option>
						</select>
						<select class="fix_width125" id="output_size$param[idx]">
							<option value="">사이즈(전체)</option>
						</select>
						<label class="fix_width10"></label>
						*
						...
						<label class="fix_width10"></label>
						<input type="text" class="input_co2 fix_width100" id="output_price$param[idx]">
...
function makePrintLowHtml($param) {
    ...
    $html = <<<html
					<div class="form-group" id="print_group$param[idx]">
						<label class="control-label fix_width120 tar">인쇄</label><label class="fix_width20 fs14 tac">:</label>
						<select class="fix_width180" id="cate_sortcode$param[idx]" onchange="selectOptionVal.exec('print', 'NAME', '$param[idx]')">
                          $param[print_option_html]
						</select>
						<select class="fix_width220" id="print_name$param[idx]" onchange="selectOptionVal.exec('print', 'PURP', '$param[idx]')">
							<option value="">인쇄명(전체)</option>
						</select>
						...
						*
						...
						<label class="fix_width10"></label>
						<input type="text" class="input_co2 fix_width100" id="print_price$param[idx]">
...
function makeAfterLowHtml($param) {
    ...
    $html = <<<html
					<div class="form-group" id="after_group$param[idx]">
						<label class="control-label fix_width120 tar">후공정</label><label class="fix_width20 fs14 tac">:</label>
						<select class="fix_width130" id="after_name$param[idx]" onchange="selectOptionVal.exec('after', 'DEPTH1', '$param[idx]')">
                          $param[after_option_html]
						</select>
						<select class="fix_width130" id="after_depth_one$param[idx]" onchange="selectOptionVal.exec('after', 'DEPTH2', '$param[idx]')">
							<option value="">Depth1(전체)</option>
						</select>
						<select class="fix_width130" id="after_depth_two$param[idx]" onchange="selectOptionVal.exec('after', 'DEPTH3', '$param[idx]')">
							<option value="">Depth2(전체)</option>
						</select>
						<select class="fix_width130" id="after_depth_thr$param[idx]">
							<option value="">Depth3(전체)</option>
						</select>
						<label class="fix_width10"></label>
						*
						...
						<label class="fix_width10"></label>
						<input type="text" class="input_co2 fix_width100" id="after_price$param[idx]">
...
function makeEtcLowHtml($param) {
    ...
    $html = <<<html
					<div class="form-group" id="etc_group$param[idx]">
						<label class="control-label fix_width120 tar">기타</label><label class="fix_width20 fs14 tac">:</label>
						<input type="text" class="input_co2 fix_width697" id="etc_cont$param[idx]">
						<label class="fix_width10"></label>
						<input type="text" class="input_co2 fix_width100" id="etc_price$param[idx]">
