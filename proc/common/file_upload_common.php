...
if($fb->fb['type'] == 'user' && $fb->fb['ftype'] == 'order'){
    $up_detail_path = ORDER_USERUPLOAD_FILE;
}

$save_file_path = ORDER_USERUPLOAD_FILE . "/" .date('Y').date('m').date('d')."/";
...
$fileName = date(Ymdhis) . "_" . mt_rand(100, 999);
...
if (!$chunks || $chunk == $chunks - 1) {
    ...
    $seqno = $dao->setOrderImg($conn,$param);
}

// Return Success JSON-RPC response
die('{"jsonrpc" : "2.0", "result" : null, "id" : ' .$seqno .'}');
